...
public:
	...
	constexpr Color( int r, int g, int b, int a = 255 ) :
		m_arrColor( { static_cast< std::uint8_t >( r ), static_cast< std::uint8_t >( g ), static_cast< std::uint8_t >( b ), static_cast< std::uint8_t >( a ) } ) { }

	/* float color constructor (in: 0.0 - 1.0) */
	constexpr Color( float r, float g, float b, float a = 1.0f ) :
		m_arrColor( { static_cast< std::uint8_t >( r * 255.f ), static_cast< std::uint8_t >( g * 255.f ), static_cast< std::uint8_t >( b * 255.f ), static_cast< std::uint8_t >( a * 255.f ) } ) { }

	/* output color to given variables */
	void Get( std::uint8_t& r, std::uint8_t& g, std::uint8_t& b, std::uint8_t& a ) const
	{
		r = m_arrColor[ COLOR_R ];
		g = m_arrColor[ COLOR_G ];
		b = m_arrColor[ COLOR_B ];
		a = m_arrColor[ COLOR_A ];
	}

	/* convert color to directx argb */
	[[nodiscard]] D3DCOLOR GetD3D( ) const
	{
		return D3DCOLOR_ARGB( m_arrColor[ COLOR_A ], m_arrColor[ COLOR_R ], m_arrColor[ COLOR_G ], m_arrColor[ COLOR_B ] );
	}
	...
	{
		return this->m_arrColor == colSecond.m_arrColor;
	}
	...
	{
		m_arrColor[ COLOR_R ] = colFrom.m_arrColor[ COLOR_R ];
		m_arrColor[ COLOR_G ] = colFrom.m_arrColor[ COLOR_G ];
		m_arrColor[ COLOR_B ] = colFrom.m_arrColor[ COLOR_B ];
		m_arrColor[ COLOR_A ] = colFrom.m_arrColor[ COLOR_A ];
		return *this;
	}
	...
	[[nodiscard]] Color Set( const std::uint8_t nValue ) const
	{
		...
		Color colCopy = *this;
		colCopy.m_arrColor[ N ] = nValue;
		return colCopy;
	}

	void Set( std::uint8_t r, std::uint8_t g, std::uint8_t b, std::uint8_t a )
	{
		m_arrColor[ COLOR_R ] = r;
		m_arrColor[ COLOR_G ] = g;
		m_arrColor[ COLOR_B ] = b;
		m_arrColor[ COLOR_A ] = a;
	}

	void Set( float r, float g, float b, float a )
	{
		m_arrColor[ COLOR_R ] = ( std::uint8_t )( r * 255.f );
		m_arrColor[ COLOR_G ] = ( std::uint8_t )( g * 255.f );
		m_arrColor[ COLOR_B ] = ( std::uint8_t )( b * 255.f );
		m_arrColor[ COLOR_A ] = ( std::uint8_t )( a * 255.f );
	}

	inline std::uint8_t R( ) const { return m_arrColor[ COLOR_R ]; }
	inline std::uint8_t G( ) const { return m_arrColor[ COLOR_G ]; }
	inline std::uint8_t B( ) const { return m_arrColor[ COLOR_B ]; }
	inline std::uint8_t A( ) const { return m_arrColor[ COLOR_A ]; }
	...
	[[nodiscard]] Color Multiplier( const float flValue ) const
	{
		...
		Color colCopy = *this;
		colCopy.m_arrColor[ N ] = static_cast< std::uint8_t >( static_cast< float >( colCopy.m_arrColor[ N ] ) * flValue );
		return colCopy;
	}
	...
	{
		if ( m_arrColor[ COLOR_R ] == m_arrColor[ COLOR_G ] && m_arrColor[ COLOR_G ] == m_arrColor[ COLOR_B ] )
			return 0.f;
...
private:
	std::array<std::uint8_t, 4U> m_arrColor;
};