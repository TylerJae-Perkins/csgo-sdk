...
void ImGui::Render()
{
    ...
    {
        ImVec4 m_vecMenuTheme = ImGui::GetStyleColorVec4( ImGuiCol_Mouse );
        RenderMouseCursor( &g.ForegroundDrawList, g.IO.MousePos, g.Style.MouseCursorScale, g.MouseCursor,  IM_COL32( m_vecMenuTheme.x * 255, m_vecMenuTheme.y * 255, m_vecMenuTheme.z * 255, m_vecMenuTheme.w * 255 ), IM_COL32( 0, 0, 0, m_vecMenuTheme.w * 255 ), IM_COL32( 0, 0, 0, std::min( 40.f, m_vecMenuTheme.w * 255 ) ) );
    }
...
static void ImGui::RenderWindowOuterBorders(ImGuiWindow* window)
{
    ...
    if (border_size > 0.0f && !(window->Flags & ImGuiWindowFlags_NoBackground))
        window->DrawList->AddRect(window->Pos, window->Pos + window->Size, GetColorU32( ImGuiCol_BorderWindow ), rounding, ImDrawCornerFlags_All, border_size);
    ...
    if (g.Style.FrameBorderSize > 0 && !(window->Flags & ImGuiWindowFlags_NoTitleBar))
    {
        ...
        window->DrawList->AddLine(ImVec2(window->Pos.x + border_size, y), ImVec2(window->Pos.x + window->Size.x - border_size, y), GetColorU32( ImGuiCol_BorderWindow ), g.Style.FrameBorderSize);
    }
}
    ...
    else
    {
        ...
        if (flags & ImGuiWindowFlags_MenuBar)
        {
            ...
            if (style.FrameBorderSize > 0.0f && menu_bar_rect.Max.y < window->Pos.y + window->Size.y)
                window->DrawList->AddLine(menu_bar_rect.GetBL(), menu_bar_rect.GetBR(), GetColorU32( ImGuiCol_BorderWindow ), style.FrameBorderSize);
        }
