...
#include "../../src/config/variables.h"
...
bool ImGui::ImageButton(ImTextureID user_texture_id,const char* alt_txt, const ImVec2& size,bool selected, const ImVec2& uv0, const ImVec2& uv1, int frame_padding, const ImVec4& bg_col, const ImVec4& tint_col)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    ImGuiContext& g = *GImGui;
    const ImGuiStyle& style = g.Style;
    const ImVec2 label_size = CalcTextSize(alt_txt, NULL, true);

    const ImVec4 m_veMenuTheme = ImVec4( Variables::m_colMenuTheme.Base<COLOR_R>( ), Variables::m_colMenuTheme.Base<COLOR_G>( ), Variables::m_colMenuTheme.Base<COLOR_B>( ), Variables::m_colMenuTheme.Base<COLOR_A>( ) );
    ...
    if (user_texture_id != nullptr)
        ...
        RenderTextClipped(bb.Min, bb.Max, alt_txt, NULL, &label_size, ImVec2(0.5f, 0.5f), &bb);

    ImGui::GetWindowDrawList()->AddRect(bb.Min, bb.Max, GetColorU32(ImVec4( m_veMenuTheme.x, m_veMenuTheme.y, m_veMenuTheme.z, std::min( m_veMenuTheme.z, !selected ? it_hover->second : 1.f) ) ), style.FrameRounding);
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImRect check_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(label_size.y + style.FramePadding.y * 1.5f, label_size.y + style.FramePadding.y * 1.8f)); // We want a square shape to we use Y twice
    ...
    auto clr = GetColorU32(ImGuiCol_ControlBg);

    static std::unordered_map<ImGuiID, float> text_hover_animation;
    auto txt_hover = text_hover_animation.find(id);
    if (txt_hover == text_hover_animation.end())
    {
        text_hover_animation.insert({ id, 0.f });
        txt_hover = text_hover_animation.find(id);
    }
    txt_hover->second = ImClamp(txt_hover->second + (3.f * ImGui::GetIO().DeltaTime * (hovered ? 1.f : -1.f)), 0.5f, 1.f);
    ...
    RenderFrame(check_bb.Min, check_bb.Max, clr, true, 1.5f);
    ImGui::GetWindowDrawList()->AddRect(check_bb.Min, check_bb.Max, GetColorU32(ImVec4( m_veMenuTheme.x, m_veMenuTheme.y, m_veMenuTheme.z, std::min( m_veMenuTheme.w, it_hover->second ) ) ), 1.5f);
    ...
    if ( *v || changed )
        window->DrawList->AddRectFilled(check_bb.Min + ImVec2(1, 1), check_bb.Max - ImVec2(1, 1), GetColorU32(ImGuiCol_CheckMark), 1.5f);
    if ( changed )
        window->DrawList->AddRectFilled(check_bb.Min + ImVec2(size[fid] * 0.5f, size[fid] * 0.5f), check_bb.Max - ImVec2(size[fid] * 0.5f, size[fid] * 0.5f), GetColorU32(ImGuiCol_FrameBg), 1.5f);

    PushClipRect(check_bb.Min, check_bb.Min + ImVec2(size2[fid], check_bb.GetHeight()), false);
    if (*v)
        RenderCheckMark(check_bb.Min + ImVec2(pad, pad), GetColorU32(ImVec4(0.1f, 0.1f, 0.1f, 1.f)), check_bb.GetWidth() - pad * 2.0f);
    PopClipRect();
    ...
    if (label_size.x > 0.0f) {
        PushStyleColor(ImGuiCol_Text, *v ? ImVec4(1.f, 1.f, 1.f, 1.f) : ImVec4(1.f, 1.f, 1.f, txt_hover->second));
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const float expected_w = CalcItemWidth() + 98.f;
    const float w = (flags & ImGuiComboFlags_NoPreview) ? arrow_size : expected_w;
    const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w, (label_size.y * 1.5f ) + style.FramePadding.y));
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? (style.ItemInnerSpacing.x * 4.f) + label_size.x : 0.0f, 0.0f));
    ItemSize(total_bb, style.FramePadding.y);
    ...
    //0, 130, 82
    const ImVec4 hover_act = m_veMenuTheme;
    const ImVec4 hover_dis = ImVec4( m_veMenuTheme.x, m_veMenuTheme.y, m_veMenuTheme.z, 0.4f);;

    float deltatime = 1.5f * ImGui::GetIO().DeltaTime;

    static std::unordered_map<ImGuiID, ImVec4> hover_animation;
    auto it_hover = hover_animation.find(id);
    if (it_hover == hover_animation.end())
    {
    ...
    const ImVec4 arrow_act = m_veMenuTheme;
    const ImVec4 arrow_dis = ImVec4(150 / 255.f, 150 / 255.f, 150 / 255.f, 1.f);;
    static std::unordered_map<ImGuiID, ImVec4> arrow_col_animation;
    ...
    static std::unordered_map<ImGuiID, arrow_anim> arrow_alpha;
    ...
    static std::unordered_map<ImGuiID, circle_button> circle_animation;
    ...
    static std::unordered_map<ImGuiID, bool> scoll_set;
    ...
    if (pressed || it_circle->second.active)
    {
        ...
        it_circle->second.alpha = ImClamp(it_circle->second.alpha - (2.f * ImGui::GetIO().DeltaTime), 0.0f, 0.8f);
        it_circle->second.radius = ImClamp(it_circle->second.radius + (240.f * ImGui::GetIO().DeltaTime), 0.0f, 400.f);
    ...
    if (it_arrow_alpha->second.open != popup_open)
    {
        if (!it_arrow_alpha->second.reverse)
        {
            ...
            it_arrow_alpha->second.alpha = ImClamp(it_arrow_alpha->second.alpha - (8.4f * ImGui::GetIO().DeltaTime), 0.01f, 1.f);
        }
        else
        {
            ...
            it_arrow_alpha->second.alpha = ImClamp(it_arrow_alpha->second.alpha + (2.4f * ImGui::GetIO().DeltaTime), 0.01f, 1.f);
        }
    }
    else
    {
    ...
    if (!(flags & ImGuiComboFlags_NoArrowButton))
    {
        ...
        PushFont(Fonts::Icons);
        window->DrawList->AddText(ImVec2(frame_bb.Max.x - 24.f, frame_bb.Min.y + ( style.FramePadding.y + 4.f ) ), text_col, /*popup_open ? "," :*/ ")" );
        PopFont();
    }

    window->DrawList->PushClipRect(frame_bb.Min, frame_bb.Max, false);
    ImGui::GetWindowDrawList()->AddCircleFilled(it_circle->second.pos, it_circle->second.radius, GetColorU32(ImVec4( m_veMenuTheme.x, m_veMenuTheme.y, m_veMenuTheme.z, std::min( m_veMenuTheme.w,it_circle->second.alpha) ) ), 60 );
    window->DrawList->PopClipRect();

    window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, GetColorU32(it_hover->second), style.FrameRounding);

    if (preview_value != NULL && !(flags & ImGuiComboFlags_NoPreview))
        RenderTextClipped(frame_bb.Min + style.FramePadding, ImVec2(value_x2, frame_bb.Max.y), preview_value, NULL, NULL, ImVec2(0.f, 0.5f));
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, float power)
{
    ...
    const ImGuiStyle& style = g.Style;
    const ImGuiID id = window->GetID(label);
    const float w = CalcItemWidth() + 115.f;
    ...
    ImGui::GetWindowDrawList()->AddRect(frame_bb.Min, frame_bb.Max, GetColorU32(ImVec4(0 / 255.f, 158 / 255.f, 100 / 255.f, it_hover->second)), g.Style.FrameRounding);
    ...
    if (grab_bb.Max.x > grab_bb.Min.x)
    {
        ...
        // gradient
        window->DrawList->AddRectFilledMultiColor(grab_bb.Min, grab_bb.Max, GetColorU32(ImVec4(0.0f, 0.0f, 0.0f, 0.75f)), GetColorU32(ImVec4(0.0f, 0.0f, 0.0f, 0.15f)), GetColorU32(ImVec4(0.0f, 0.0f, 0.0f, 0.15f)), GetColorU32(ImVec4(0.0f, 0.0f, 0.0f, 0.75f)) );
    }
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg, bool alignCenter, bool renderBullet, ImVec4 bulletCol)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    const ImVec4 m_veMenuTheme = ImVec4( Variables::m_colMenuTheme.Base<COLOR_R>( ), Variables::m_colMenuTheme.Base<COLOR_G>( ), Variables::m_colMenuTheme.Base<COLOR_B>( ), Variables::m_colMenuTheme.Base<COLOR_A>( ) );
    const ImVec4 text_act = m_veMenuTheme;
    ...
    if (renderBullet)
        window->DrawList->AddCircleFilled(pos + ImVec2(style.FramePadding.x + g.FontSize * 0.125f, line_height * 0.5f), g.FontSize * 0.30f, GetColorU32(bulletCol), 4);

    PushStyleColor(ImGuiCol_Text, it_hcolor->second);
    RenderTextClipped(alignCenter ? text_min : ImVec2(text_min.x + 20.f, text_min.y), text_max, label, NULL, &label_size, alignCenter ? ImVec2(0.5f, 0.5f) : ImVec2(0.f, 0.5f), &bb_enlarged);
    PopStyleColor();
...
bool ImGui::BeginMainMenuBar(float x, float y)
{
    ImGuiContext& g = *GImGui;
